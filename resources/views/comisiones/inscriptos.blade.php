...
@foreach ($comision->inscriptos as $inscripto)
    <div class="row d-flex align-items-center my-5">
        ...
        @foreach($inscriptos_comision as $insc)
            @if($insc->id_alumno == $inscripto->id)
                <div class="col text-center">
                    <label for="" class="form-label fs-3">Estado</label>
                    <input type="text" class="form-control" name="estado" value="{{$insc->estado}}" readonly>
                </div>
                <div class="col text-center">
                    <label for="" class="form-label fs-3">Nota</label>
                    <input type="text" class="form-control" name="nota" value="{{$insc->nota}}" readonly>
                </div>
            @endif
        @endforeach
            <a class="col mt-5 btn btn-info" href="/docente/comisiones/{{$comision->id}}/inscriptos/{{$inscripto->id}}">
